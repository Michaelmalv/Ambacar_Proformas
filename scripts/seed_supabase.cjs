...
// Read Supabase credentials dynamically from .env or environment
let supabaseUrl = process.env.VITE_SUPABASE_URL || "https://mblsjoreokpsjtdzjrae.supabase.co";
...
// Corrective Catalogs to extract
...
async function main() {
  console.log("================================================================");
  console.log("INICIANDO EXTRACCIÓN Y SIEMBRA DE DATOS EN SUPABASE / POSTGRESQL");
  console.log("================================================================");

  let sqlStatements = [];
  sqlStatements.push(`-- ==============================================================================`);
  sqlStatements.push(`-- DATOS SEMILLA EXTRAÍDOS DE LOS 5 ARCHIVOS EXCEL (AMBACAR)`);
  sqlStatements.push(`-- ==============================================================================`);
  sqlStatements.push(`\n-- Limpieza inicial de datos`);
  sqlStatements.push(`TRUNCATE TABLE public.plan_items_detalle CASCADE;`);
  sqlStatements.push(`TRUNCATE TABLE public.plan_costos_km CASCADE;`);
  sqlStatements.push(`TRUNCATE TABLE public.planes_mantenimiento CASCADE;`);
  sqlStatements.push(`TRUNCATE TABLE public.catalogo_correctivo CASCADE;`);
  sqlStatements.push(`TRUNCATE TABLE public.modelos_vehiculo CASCADE;\n`);

  const modelosMap = new Map();

  // 1. Process and Insert Modelos & Planes
  for (const def of planDefinitions) {
    const filePath = path.join(workspaceDir, def.file);
    if (!fs.existsSync(filePath)) {
      console.warn(`Archivo no encontrado: ${def.file}`);
      continue;
    }
    ...
    if (!ws) {
      console.warn(`Hoja [${def.sheet}] no encontrada en ${def.file}`);
      continue;
    }

    const grid = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
    console.log(`\nProcesando Plan: ${def.nombre_plan} [${def.sheet}]`);

    // Ensure Modelo in SQL
    if (!modelosMap.has(def.nombre_completo)) {
      const vehiculoUuid = `veh_${modelosMap.size + 1}`;
      modelosMap.set(def.nombre_completo, vehiculoUuid);
      sqlStatements.push(`INSERT INTO public.modelos_vehiculo (marca, modelo, motor, traccion, nombre_completo) VALUES ('${def.marca}', '${def.modelo}', '${def.motor}', '${def.traccion}', '${def.nombre_completo}') ON CONFLICT (nombre_completo) DO NOTHING;`);
    }

    // Identify header row with KMs
    let kmRowIndex = null;
    let kmCols = [];
    ...
    if (!kmCols.length) {
      console.warn(`No se encontraron columnas de KM en ${def.sheet}`);
      continue;
    }
    ...
    sqlStatements.push(`INSERT INTO public.planes_mantenimiento (vehiculo_id, codigo_plan, nombre, archivo_origen, hoja_origen, km_inicio, km_fin, intervalo_km) VALUES ((SELECT id FROM public.modelos_vehiculo WHERE nombre_completo = '${def.nombre_completo}'), '${def.codigo_plan}', '${def.nombre_plan}', '${def.file}', '${def.sheet}', ${kmMin}, ${kmMax}, 5000) ON CONFLICT (codigo_plan) DO NOTHING;`);

    // Find Totals rows in sheet
    ...
    for (let r = 0; r < grid.length; r++) {
      ...
      if (c0.includes('TOTAL LUBRICANTE') || c0 === 'TOTAL LUBRICANTES') {
        filaLub = r;
        currentCategoria = 'REPUESTOS';
        continue;
      }
      if (c0.includes('TOTAL REPUESTO') || c0 === 'TOTAL REPUESTOS') {
        filaRep = r;
        currentCategoria = 'MANO DE OBRA';
        continue;
      }
      if (c0.includes('TOTAL MANO DE OBRA') || c0.includes('TOTAL M/O')) {
        filaMo = r;
        continue;
      }
      if (c0.includes('TOTAL COSTO') || c0.includes('TOTAL MANTENIMIENTO') || c0 === 'TOTAL') {
        filaTot = r;
        continue;
      }

      // If it is an item row
      if (r > kmRowIndex && c0 && !c0.includes('PLAN DE') && !c0.includes('MARCA') && !c0.includes('MODELO')) {
        ...
        if (hasNumbers) {
          ...
          if (c0.includes('ACEITE') || c0.includes('LUBRIC') || c0.includes('LIQUIDO') || c0.includes('REFRIGERANTE')) {
            cat = 'LUBRICANTES';
          }
    ...
    console.log(`   KMs: ${kmCols.length} mantenimientos (${kmMin} - ${kmMax} KM) | Filas Totales: Rep=${filaRep}, Lub=${filaLub}, MO=${filaMo}`);

    // Generate Costos por KM SQL
    for (const { col, km } of kmCols) {
      ...
      sqlStatements.push(`INSERT INTO public.plan_costos_km (plan_id, km, total_repuestos, total_lubricantes, total_mano_obra, total_km) VALUES ((SELECT id FROM public.planes_mantenimiento WHERE codigo_plan = '${def.codigo_plan}'), ${km}, ${rep}, ${lub}, ${mo}, ${tot}) ON CONFLICT (plan_id, km) DO UPDATE SET total_repuestos = EXCLUDED.total_repuestos, total_lubricantes = EXCLUDED.total_lubricantes, total_mano_obra = EXCLUDED.total_mano_obra, total_km = EXCLUDED.total_km;`);
      ...
      for (const item of itemsList) {
        ...
        if (aplica) {
          const descEscaped = item.descripcion.replace(/'/g, "''");
          sqlStatements.push(`INSERT INTO public.plan_items_detalle (plan_id, categoria, descripcion, km, costo, aplica) VALUES ((SELECT id FROM public.planes_mantenimiento WHERE codigo_plan = '${def.codigo_plan}'), '${item.categoria}', '${descEscaped}', ${km}, ${num}, ${aplica});`);
        }
      }
    }
  }

  // 2. Process and Insert Corrective Catalogs
  console.log(`\nProcesando Catálogos de Mantenimiento Correctivo...`);
  for (const corr of correctiveDefinitions) {
    ...
    const grid = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
    console.log(`   Catálogo: ${corr.modelo_referencia} [${corr.sheet}]`);
    ...
    for (let r = startRow; r < grid.length; r++) {
      ...
      if (!desc || desc.toUpperCase().includes('TOTAL') || desc.toUpperCase().includes('SUBTOTAL') || desc.toUpperCase().includes('NOTA')) {
        continue;
      }
      ...
      sqlStatements.push(`INSERT INTO public.catalogo_correctivo (modelo_referencia, tipo_catalogo, item_numero, descripcion, costo_repuestos, costo_mano_obra, costo_lubricantes, subtotal, iva, total) VALUES ('${corr.modelo_referencia}', '${corr.tipo_catalogo}', ${isNaN(itemNum) ? 'NULL' : itemNum}, '${descEsc}', ${rep}, ${mo}, ${lub}, ${sub}, ${iva}, ${tot});`);
    }
  }

  // Write seed.sql
  const seedSqlPath = path.join(workspaceDir, 'supabase', 'seed.sql');
  fs.writeFileSync(seedSqlPath, sqlStatements.join('\n'), 'utf-8');
  console.log(`\n✅ Archivo SQL generado exitosamente: ${seedSqlPath} (${sqlStatements.length} sentencias SQL)`);

  if (supabase) {
    console.log(`\nVerificando conexión con Supabase API...`);
    try {
      const { data: testData, error: testErr } = await supabase.from('modelos_vehiculo').select('count', { count: 'exact' });
      if (testErr) {
        console.log(`Nota: Ejecuta supabase/setup_complete.sql en tu consola de Supabase SQL Editor para crear las tablas.`);
      } else {
        console.log(`Conexión con Supabase verificada exitosamente.`);
      }
    } catch (apiErr) {
      console.log(`Verificación de API: ${apiErr.message}`);
    }
  }

  console.log("\n================================================================");
  console.log("PROCESO DE EXTRACCIÓN Y GENERACIÓN COMPLETADO CON ÉXITO");
  console.log("================================================================");
}

main().catch(err => {
  console.error("Error ejecutando script de extracción:", err);
  process.exit(1);
});
