...
export default function App() {
  ...
  // Parse Maintenance Plan Template File
  const handleTemplateChange = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Auto detect sheet matching 4X4 / 4X2
        ...
        if (detected) {
          setSheetSelection(detected);
          setSheetAutoDetected(true);
        } else {
          setSheetSelection(workbook.SheetNames[0] || "");
          setSheetAutoDetected(false);
        }
      } catch (err) {
  ...
  // Handle image uploads (converting to base64)
  ...
  // Remove local storage cached images
  ...
  // Recalculate cost when range, sheet, vehicles, or correctivos change
  useEffect(() => {
    if (!templateWorkbook || !sheetSelection) return;
    ...
    try {
      ...
      const importXLSX = async () => {
        ...
        const res = calcularPlanMantenimiento(
          planGrid,
          parseInt(form.kmDesde, 10) || 0,
          parseInt(form.kmHasta, 10) || 0,
          parseInt(form.cantidadVehiculos, 10) || 1
        );
        
        if (res.error) {
          setCalcResult(null);
          setCalcError(`No se encontraron kilómetros válidos en el rango [${form.kmDesde || 0} - ${form.kmHasta || 0}] para esta hoja. KMs en plantilla: ${res.kmsDisponibles.join(', ')}`);
        } else {
          setCalcResult(res);
          setCalcError(null);
        }
      ...
      setCalcError("Error al procesar los cálculos de mantenimiento.");
    }
  }, [templateWorkbook, sheetSelection, form.kmDesde, form.kmHasta, form.cantidadVehiculos]);
  ...
  // Trigger Word docx Generation
  const handleGenerateWord = async () => {
    if (!calcResult) return;
    setIsGenerating(true);
    
    // Prepare complete calculations data
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        <div className="brand-section">
          ...
          <div>
            ...
            <p className="brand-subtitle">Procesador automático de cotizaciones y planes de mantenimiento</p>
          </div>
        </div>
        <div className="brand-badge">WEB V1.0</div>
      </header>
      ...
      <div className="dashboard-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div className="card">
            ...
            <div className="form-group">
              <label className="form-label">1. Datos de la Cotización (.xlsx)</label>
              {!cotizacionFile ? (
                <div className="upload-zone" onClick={() => document.getElementById('cot-input').click()}>
                  <UploadCloud className="upload-icon" />
                  <p className="upload-text">Selecciona o arrastra el archivo de cotización</p>
            ...
            {/* Template Plan upload */}
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="form-label">2. Plantilla Plan de Mantenimiento (.xlsx)</label>
              {!templateFile ? (
                <div className="upload-zone" onClick={() => document.getElementById('template-input').click()}>
                  <UploadCloud className="upload-icon" />
                  <p className="upload-text">Selecciona la plantilla de precios</p>
                  <p className="upload-subtext">Archivo de plan preventivo oficial</p>
                  <input 
                    type="file" 
                    id="template-input" 
                    style={{ display: 'none' }} 
                ...
                <div className="file-pill">
                  ...
                  <button className="file-pill-remove" onClick={() => { setTemplateFile(null); setTemplateWorkbook(null); setHojas([]); setSheetSelection(""); setCalcResult(null); }}>
                    <X size={14} />
                  </button>
                </div>
              )}
          ...
          <div className="card">
            ...
            <div className="image-uploader-container">
              {/* Header Box */}
              <div>
                <label className="form-label">Cabecera (Header Logo)</label>
                {headerImg ? (
            ...
            {(!headerImg || !footerImg) && (
              <p className="upload-subtext" style={{ marginTop: '0.75rem', color: '#f59e0b', display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
                <AlertTriangle size={12} />
                Nota: Se usarán textos básicos si no cargas las imágenes oficiales de Ambacar.
              </p>
            )}
          </div>

          {/* Card 3: Datos del Cliente & Vehiculo */}
          <div className="card">
            <h3 className="card-title">
        ...
        <div className="preview-container">
          ...
          {/* Card: Sheet Selection */}
          <div className="card">
            <h3 className="card-title">
              <FileCode size={20} className="text-muted" />
              Selección de Hoja de Precios
            </h3>
            
            {!templateWorkbook ? (
              <div className="alert alert-warning" style={{ margin: 0 }}>
                <AlertTriangle size={18} />
                <span>Sube la plantilla de precios para listar las hojas disponibles.</span>
              </div>
            ) : (
              <div>
                <div className="form-group">
                  <label className="form-label">Seleccionar Hoja:</label>
                  <select 
                    className="form-input" 
                    value={sheetSelection} 
                    onChange={(e) => { setSheetSelection(e.target.value); setSheetAutoDetected(false); }}
                  >
                    {hojas.map(name => (
                ...
                {sheetAutoDetected && (
                  <div className="alert alert-success" style={{ margin: 0 }}>
                    ...
                    <span>Hoja auto-detectada para el vehículo <strong>{form.modeloVehiculo}</strong>.</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="card" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {!calcResult && !calcError && (
              <div className="alert alert-warning" style={{ margin: 0, flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <AlertTriangle size={18} />
                <span>Faltan archivos o datos válidos para procesar el cálculo.</span>
              </div>
            )}
          ...
          <div className="card" style={{ padding: '1rem', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            >
              <Download size={18} />
              {isGenerating ? 'Generando Documento...' : 'Descargar Proforma (.docx)'}
            </button>
          </div>
      ...
      <footer className="footer-credits">
        Generador de Proformas Ambacar &copy; {new Date().getFullYear()}. Todos los derechos reservados. Desarrollado en React.
      </footer>
    </div>
  );
}
